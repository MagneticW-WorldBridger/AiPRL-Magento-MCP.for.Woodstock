...
// Date parsing utilities (EXACT COPY from original)
...
// Create an MCP server (EXACT COPY from original)
...
// Format product data for better readability (EXACT COPY from original)
...
// Format search results for better readability (EXACT COPY from original)
...
// ALL ORIGINAL 13 TOOLS (EXACT COPY from original)
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    tools: 'Original 13 tools loaded'
  });
});
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Magento MCP Server (SSE) running on port ${PORT}`);
